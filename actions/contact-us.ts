...
export const contactUs = async (values: z.infer<typeof ContactUsSchema>) => {
  ...
  try {
    ...
    if (!validatedFields.success) {
      ...
      if (firstError) {
        ...
        if (firstError.field === "name") {
          ...
          errorMessage =
            "Please provide a description (at least 6 characters).";
    ...
    const { name, email, reason, helpDescription, website, recaptchaToken } =
    ...
    if (!validateHoneypot(website)) {
      ...
      console.warn(
        `[CONTACT_US] Bot detected via honeypot field`,
        errorDetails
      );
    ...
    for (let attempt = 0; attempt <= maxRetries; attempt++) {
      try {
        ...
        console.warn(
          `[CONTACT_US] Database error, retrying in ${delayMs}ms...`,
          errorDetails
        );
    ...
    console.error(
      `[CONTACT_US] Database error after ${retryCount + 1} attempts`,
      errorDetails
    );
    ...
    if (lastError instanceof Error) {
      if (lastError.message.includes("Unique constraint")) {
        return {
          error:
            "A submission with this information already exists. Please wait a moment before trying again.",
          retryable: false,
        };
      }
      if (
    ...
    return {
      error:
        "Failed to submit your request after multiple attempts. Please try again.",
      retryable: true,
    };
  } catch (error) {
    const totalDuration = Date.now() - startTime;
    ...
    console.error(
      `[CONTACT_US] Unexpected error in contact form submission`,
      errorDetails
    );
