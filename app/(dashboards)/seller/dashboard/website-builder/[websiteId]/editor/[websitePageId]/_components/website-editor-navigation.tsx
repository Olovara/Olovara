...
import { saveActivityLogsNotification, upsertWebsitePage } from "@/lib/queries";
...
const WebsiteEditorNavigation = ({
  ...
  const handleOnBlurTitleChange: FocusEventHandler<HTMLInputElement> = async (
    ...
    if (event.target.value === websitePageDetails.name) return;
    if (event.target.value) {
      await upsertWebsitePage(
        sellerId,
        {
          id: websitePageDetails.id,
          name: event.target.value,
          order: websitePageDetails.order,
        },
        websiteId
      );

      toast("Success", {
        description: "Saved Website Page title",
      });
      router.refresh();
    } else {
      toast("Oops!", {
      ...
      event.target.value = websitePageDetails.name;
    }
  };
        ...
        <aside className="flex items-center gap-2">
          ...
          <div className="flex flex-col item-center mr-4">
            ...
            <span className="text-muted-foreground text-sm">
              Last updated {websitePageDetails.updatedAt.toLocaleDateString()}
            </span>
          </div>
