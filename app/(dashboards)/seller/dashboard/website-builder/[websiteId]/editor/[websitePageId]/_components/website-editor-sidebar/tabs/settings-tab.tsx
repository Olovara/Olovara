    ...
    >
      ...
      >
        ...
        <AccordionContent>
          ...
          {/* Navbar Block Settings */}
          {state.editor.selectedElement.type === 'navbar' &&
            !Array.isArray(state.editor.selectedElement.content) && (
              <div className="flex flex-col gap-4">
                <div className="flex flex-col gap-2">
                  ...
                  <Input
                    ...
                    onChange={handleChangeCustomValues}
                    value={state.editor.selectedElement.content.shopName || ''}
                  />
                </div>
                <div className="flex flex-col gap-2">
                  <p className="text-muted-foreground">Show Buttons</p>
                  <div className="flex gap-4">
                    <label className="flex items-center gap-2">
                      <input
                        type="checkbox"
                        checked={state.editor.selectedElement.content.showWishlist !== false}
                        onChange={(e) => handleChangeCustomValues({
                          target: { id: 'showWishlist', value: e.target.checked }
                        })}
                      />
                      Wishlist
                    </label>
                    <label className="flex items-center gap-2">
                      <input
                        type="checkbox"
                        checked={state.editor.selectedElement.content.showCart !== false}
                        onChange={(e) => handleChangeCustomValues({
                          target: { id: 'showCart', value: e.target.checked }
                        })}
                      />
                      Cart
                    </label>
                    <label className="flex items-center gap-2">
                      <input
                        type="checkbox"
                        checked={state.editor.selectedElement.content.showAccount !== false}
                        onChange={(e) => handleChangeCustomValues({
                          target: { id: 'showAccount', value: e.target.checked }
                        })}
                      />
                      Account
                    </label>
          ...
          {/* Hero Block Settings */}
          {state.editor.selectedElement.type === 'hero' &&
            !Array.isArray(state.editor.selectedElement.content) && (
              <div className="flex flex-col gap-4">
                <div className="flex flex-col gap-2">
                  ...
                  <Input
                    ...
                    onChange={handleChangeCustomValues}
                    value={state.editor.selectedElement.content.heading || ''}
                  />
                </div>
                <div className="flex flex-col gap-2">
                  ...
                  <Input
                    ...
                    onChange={handleChangeCustomValues}
                    value={state.editor.selectedElement.content.subheading || ''}
                  />
                </div>
                <div className="flex flex-col gap-2">
                  <p className="text-muted-foreground">CTA Button Text</p>
                  <Input
                    id="ctaText"
                    placeholder="Shop Now"
                    onChange={handleChangeCustomValues}
                    value={state.editor.selectedElement.content.ctaText || ''}
                  />
                </div>
                <div className="flex flex-col gap-2">
                  <p className="text-muted-foreground">CTA Button Link</p>
                  <Input
                    id="ctaLink"
                    placeholder="/shop"
                    onChange={handleChangeCustomValues}
                    value={state.editor.selectedElement.content.ctaLink || ''}
                  />
                </div>
                <div className="flex flex-col gap-2">
                  <p className="text-muted-foreground">Background Type</p>
                  <Select
                    onValueChange={(value) => handleChangeCustomValues({
                      target: { id: 'backgroundType', value }
                    })}
                    value={state.editor.selectedElement.content.backgroundType || 'color'}
                  >
                    <SelectTrigger>
                ...
                {state.editor.selectedElement.content.backgroundType === 'color' && (
                  <div className="flex flex-col gap-2">
                    ...
                    <div className="flex border-[1px] rounded-md overflow-clip">
                      <div
                        className="w-12"
                        style={{
                          backgroundColor: state.editor.selectedElement.content.backgroundColor || '#f3f4f6'
                        }}
                      />
                      <Input
                        placeholder="#f3f4f6"
                        ...
                        id="backgroundColor"
                        onChange={handleChangeCustomValues}
                        value={state.editor.selectedElement.content.backgroundColor || ''}
                      />
                    </div>
                  </div>
                )}
                {state.editor.selectedElement.content.backgroundType === 'image' && (
                  <div className="flex flex-col gap-2">
                    <p className="text-muted-foreground">Background Image URL</p>
                    <Input
                      id="backgroundImage"
                      placeholder="https://example.com/image.jpg"
                      onChange={handleChangeCustomValues}
                      value={state.editor.selectedElement.content.backgroundImage || ''}
                    />
                  </div>
                )}
              </div>
          ...
          {state.editor.selectedElement.type === 'products' &&
            !Array.isArray(state.editor.selectedElement.content) && (
              <div className="flex flex-col gap-2">
          ...
          {state.editor.selectedElement.type === 'footer' &&
            !Array.isArray(state.editor.selectedElement.content) && (
              <div className="flex flex-col gap-4">
                <p className="text-muted-foreground">Footer Columns</p>
                <div className="space-y-4">
                  {(state.editor.selectedElement.content as any).columns?.map((column: any, columnIndex: number) => (
                    <div key={columnIndex} className="border rounded-lg p-3">
                      <div className="flex flex-col gap-2 mb-3">
                        <p className="text-sm font-medium">Column {columnIndex + 1} Header</p>
                        <Input
                          id={`column_${columnIndex}_header`}
                          placeholder="Column Header"
                          onChange={(e) => {
                            const newColumns = [...((state.editor.selectedElement.content as any).columns || [])]
                            newColumns[columnIndex] = { ...newColumns[columnIndex], header: e.target.value }
                            handleChangeCustomValues({
                              target: { id: 'columns', value: newColumns }
                            })
                          }}
                          value={column.header || ''}
                        />
