  ...
  try {
    switch (event.type) {
      case "checkout.session.completed": {
        ...
        console.log(`✅ Processing custom order checkout.session.completed: ${session.id}`);

        if (!session.payment_intent || !session.metadata) {
          console.error("❌ Session completed without Payment Intent ID or metadata");
        ...
        const paymentIntent = await stripeWebhook.instance.paymentIntents.retrieve(session.payment_intent as string);
        if (!paymentIntent.latest_charge) {
          throw new Error("Charge ID missing from Payment Intent");
        }

        const charge = await stripeWebhook.instance.charges.retrieve(paymentIntent.latest_charge as string);
        if (charge.status !== 'succeeded') {
          console.error(`❌ Charge ${charge.id} status is ${charge.status}, not 'succeeded'`);
