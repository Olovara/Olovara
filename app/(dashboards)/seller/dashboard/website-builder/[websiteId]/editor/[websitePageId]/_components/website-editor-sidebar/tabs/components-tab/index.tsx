...
const ComponentsTab = (props: Props) => {
  ...
  return (
    <Accordion
      type="multiple"
      className="w-full"
      defaultValue={["Layout", "Elements", "Blocks"]}
    >
      ...
      <AccordionItem value="Blocks" className="px-6 py-0 ">
        <AccordionTrigger className="!no-underline">Blocks</AccordionTrigger>
        <AccordionContent className="flex flex-wrap gap-2 ">
          {elements
            .filter((element) => element.group === "blocks")
            .map((element) => (
              <div
                key={element.id}
                className="flex-col items-center justify-center flex"
