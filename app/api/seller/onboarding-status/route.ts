...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    if (seller.connectedAccountId && !seller.stripeConnected && !seller.connectedAccountId.startsWith('temp_')) {
      ...
      console.log(`[ONBOARDING_STATUS] Verifying Stripe account status (stripeConnected is false but account exists)`, logContext);
      ...
      try {
        ...
        while (retries > 0) {
          try {
            ...
            retries--;
            if (retries > 0) {
              console.warn(`[ONBOARDING_STATUS] Stripe API call failed, retrying...`, { 
                ...logContext, 
        ...
        if (!account) {
          ...
          console.error(`[ONBOARDING_STATUS] Failed to retrieve Stripe account after retries`, errorDetails);
        ...
        if (account.charges_enabled && account.payouts_enabled) {
          console.log(`[ONBOARDING_STATUS] Account fully onboarded, updating database in transaction`, logContext);
          ...
          while (transactionRetries > 0 && !transactionSuccess) {
            try {
              ...
              if (transactionRetries > 0 && isRetryable) {
                ...
                console.warn(`[ONBOARDING_STATUS] Database transaction failed, retrying...`, { 
                  ...logContext, 
          ...
          if (!transactionSuccess) {
            const errorDetails = {
              ...logContext,
              error: lastTransactionError instanceof Error ? lastTransactionError.message : String(lastTransactionError),
              errorType: lastTransactionError instanceof Error ? lastTransactionError.constructor.name : typeof lastTransactionError,
              stack: lastTransactionError instanceof Error ? lastTransactionError.stack : undefined,
              status: 'FAILED',
              note: 'Transaction failed after retries - seller.stripeConnected and onboarding step may be out of sync'
            };
            console.error(`[ONBOARDING_STATUS] Failed to update Stripe connection status in transaction after retries`, errorDetails);
            // Keep stripeConnected as false if transaction fails
          } else {
            console.log(`[ONBOARDING_STATUS] Transaction completed successfully`, logContext);

            // Recalculate isFullyActivated (outside transaction)
            try {
              await updateOnboardingStep(seller.id, "payment_setup", true);
              console.log(`[ONBOARDING_STATUS] isFullyActivated recalculated`, logContext);
            } catch (recalcError) {
              const errorDetails = {
                ...logContext,
              ...
              console.warn(`[ONBOARDING_STATUS] Failed to recalculate isFullyActivated (non-critical)`, errorDetails);
            }
            
            stripeConnected = true;
            console.log(`[ONBOARDING_STATUS] Stripe connection status updated successfully`, { 
              ...logContext, 
              status: 'SUCCESS'
            });
          }
        } else {
          console.log(`[ONBOARDING_STATUS] Account not fully onboarded yet`, logContext);
        }
      } catch (error) {
        const errorDetails = {
          ...logContext,
        ...
        console.warn(`[ONBOARDING_STATUS] Could not verify Stripe account status`, errorDetails);
    ...
    console.error(`[ONBOARDING_STATUS] Error fetching seller onboarding status`, errorDetails);
    return NextResponse.json(
