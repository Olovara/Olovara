...
export default function TermsOfService() {
  ...
  const fetchTerms = useCallback(async () => {
    try {
      const res = await fetch("/api/terms-of-service");
      if (!res.ok) {
  ...
  const handleSave = useCallback(async () => {
    ...
    setIsSaving(true);
    const toastId = toast.loading("Saving terms of service...");
    ...
    try {
      ...
      toast.error("Failed to update terms of service. Please try again.", {
        id: toastId,
      });
    } finally {
      setIsSaving(false);
      ...
      <div className="max-w-[1400px] mx-auto">
        ...
        {isAdmin && (
          <div className="mt-6 sm:mt-8 border-t pt-6 sm:pt-8">
            <h2 className="text-lg sm:text-xl font-semibold mb-4">Edit Terms of Service</h2>
            <QuillEditor
              value={content}
              onChange={setContent}
            />
